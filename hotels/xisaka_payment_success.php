...
file_put_contents('xisaka_payment_debug.log', date('Y-m-d H:i:s') . " - Xisaka Payment success called\n", FILE_APPEND);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE xisaka_bookings SET 
            payment_status = 'paid',
            payment_amount = ?,
            payment_id = ?,
            payer_email = ?,
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $bookingSql = "SELECT * FROM xisaka_bookings WHERE id = ?";
...
function sendPaymentConfirmationEmail($booking, $transactionId, $amountZAR, $amountUSD) {
    $to = $booking['email'];
    $subject = "Payment Confirmed - Xisaka Guest House (Booking #{$booking['id']})";
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                " . ($booking['addons'] ? "<div class='detail'><strong>Add-ons:</strong> {$booking['addons']}</div>" : "") . "
            ...
            <div class='footer'>
                <p>Thank you for choosing Xisaka Guest House!</p>
...
function sendPaymentNotificationToAdmin($booking, $transactionId, $amountZAR, $amountUSD, $payerEmail, $payerName) {
    $to = ADMIN_EMAIL;
    $subject = "Payment Received - Booking #{$booking['id']} - Xisaka Guest House";
