...
file_put_contents('vahlavi_payment_debug.log', date('Y-m-d H:i:s') . " - Vahlavi Payment success called\n", FILE_APPEND);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE vahlavi_bookings SET 
            payment_status = 'paid',
            payment_amount = ?,
            payment_id = ?,
            payer_email = ?,
    ...
    mysqli_stmt_bind_param($stmt, "dssi", $amountZAR, $transactionId, $payerEmail, $bookingId);
    
    if (mysqli_stmt_execute($stmt)) {
        // Get booking details for email
        $bookingSql = "SELECT * FROM vahlavi_bookings WHERE id = ?";
...
function sendPaymentConfirmationEmail($booking, $transactionId, $amountZAR, $amountUSD) {
    $to = $booking['email'];
    $subject = "Payment Confirmed - Vahlavi Guest House (Booking #{$booking['id']})";
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                " . ($booking['packages'] ? "<div class='detail'><strong>Packages:</strong> {$booking['packages']}</div>" : "") . "
            ...
            <div class='footer'>
                <p>Thank you for choosing Vahlavi Guest House!</p>
...
function sendPaymentNotificationToAdmin($booking, $transactionId, $amountZAR, $amountUSD, $payerEmail, $payerName) {
    $to = ADMIN_EMAIL;
    $subject = "Payment Received - Booking #{$booking['id']} - Vahlavi Guest House";
